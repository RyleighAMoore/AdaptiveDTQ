...
def fitQuad(Px,Py, mesh, pdf):
    ...
    c = pred_params
    A= np.asarray([[c[0], c[3]],[c[3],c[1]]])
    B=np.expand_dims(np.asarray([c[4], c[5]]),1)
...
def generate_example_data(num, params):
    ...
    xy = LP.mapPointsBack(0, 0, xy, .1, .1)
    ...
    scaling = GaussScale(2)
    scaling.setMu(np.asarray([[0,0]]).T)
    scaling.setSigma(np.asarray([.1,.1]))
    zobs = np.log(fun.Gaussian(scaling, xy.T))**2
