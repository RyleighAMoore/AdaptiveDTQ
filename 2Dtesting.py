...
# mesh = LP.generateLejaMesh(400, 1, 1, 30)
...
for i in trange(150):
    ...
    if i >-1:
        ...
        for point in range(len(mesh)):
            Px = mesh[point,0]
            Py = mesh[point,1]
            integral = u13.getNewPDFVal(Px, Py, mesh, pdf, 50, h)
            pdfNew1.append(integral[0])
        # pdf = np.asarray(pdfNew)
        # PdfTraj.append(np.copy(pdf))
        pdfNew = np.copy(pdf)                   
        print(np.linalg.norm(pdfNew1-pdfNew))
...
ax.scatter(Meshes[index][:,0], Meshes[index][:,1], PdfNewQuad[index-1], c='k', marker='.')
