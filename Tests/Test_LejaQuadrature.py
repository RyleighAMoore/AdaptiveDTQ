...
def Test_LejaQuadratureLinearizationOnLejaPointsIfAllPointsKnown1():
    h = 0.01
    sigmaX=np.sqrt(h)*g1()
    sigmaY=np.sqrt(h)*g2()
    sigma = np.sqrt(h)
...
newPDF,condNums, meshVals = Test_LejaQuadratureLinearizationOnLejaPointsIfAllPointsKnown1()
