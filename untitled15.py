...
def QuadratureByInterpolation(train_samples, train_values, sigmaX, sigmaY, muX, muY, degree):
    ...
    fig = plt.figure()
    ax = Axes3D(fig)
    ax.scatter(train_samples[:,0], train_samples[:,1],np.exp(vals1), c='r', marker='.')
    ax.scatter(train_samples[:,0], train_samples[:,1], np.exp(train_values), c='k', marker='.')
    print(np.max(np.exp(train_values)-np.exp(vals1)))
    print(coef[0])
